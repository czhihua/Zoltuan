...
//第二个参数，对象二class
//第三个参数，点击或触摸
// pullDown('box', 'son', 'onclick');
function pullDown(box, son, incident) {
    var box = document.getElementById(box);
    var son = box.getElementsByClassName(son);
    if (incident == 'onclick') {
        var isok = true;
        box.onclick = function () {
            if (isok) {
                css(son[0], 'display', 'block');
            } else {
                css(son[0], 'display', 'none');
            }
            isok = !isok;
        }
    } else if (incident == 'onmouseover') {
        box.onmouseover = function () {
            css(son[0], 'display', 'block');
        }
        box.onmouseout = function () {
...
//设置时间转毫秒
// var oldTime = (new Date("2019/03/30 20:11:11")).getTime();



//传入毫秒数转换成时分秒
function xiaoshi(my_time) {
    var days = my_time / 1000 / 60 / 60 / 24;
    var daysRound = Math.floor(days);
    var hours1 = my_time / 1000 / 60 / 60;
    var hours = my_time / 1000 / 60 / 60 - (24 * daysRound);
    var hoursRound1 = Math.floor(hours1);
    var hoursRound = Math.floor(hours);
    var minutes = my_time / 1000 / 60 - (24 * 60 * daysRound) - (60 * hoursRound);
    var minutesRound = Math.floor(minutes);
    var seconds = my_time / 1000 - (24 * 60 * 60 * daysRound) - (60 * 60 * hoursRound) - (60 * minutesRound);
    var secondsRound = Math.floor(seconds);
    // console.log('转换时间:', daysRound + '天', hoursRound + '时', minutesRound + '分', seconds + '秒');
    var time = [hoursRound1, minutesRound, secondsRound];
    return time;
}
